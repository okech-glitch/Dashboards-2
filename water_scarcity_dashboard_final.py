...
# Approximate coordinates for Kenyan counties (for Folium map) with default
coords = {
...
default_coord = [-0.0236, 37.9062]  # Central Kenya as fallback

# Map regions to coordinates with error handling
...
with col1:
    st.image(sdg13_icon, width=100, caption="SDG 13: Climate Action")
with col2:
    st.image(sdg2_icon, width=100, caption="SDG 2: Zero Hunger")
with col3:
    st.image(sdg6_icon, width=100, caption="SDG 6: Clean Water")
st.markdown("""
...
# (Rest of the demo code remains the same - region selector, custom input, visualizations)
...
for _, row in annual_df.iterrows():
    if pd.notna(row["Latitude"]) and pd.notna(row["Longitude"]) and isinstance(row["Latitude"], (int, float)) and isinstance(row["Longitude"], (int, float)):
        folium.CircleMarker(
        ...
        st.write(f"Warning: Invalid or non-numeric coordinates for {row['Region']}: Lat={row['Latitude']}, Lon={row['Longitude']}")
folium.Marker(
    location=[-0.5, 36.5],
...
try:
    ...
    if map_output.get('last_active_drawing'):
        st.write(f"Map rendered successfully with {marker_count} markers!")
    else:
        st.write("Map rendered, but no interaction detected. Check browser JavaScript settings.")
...
if os.path.exists("water_scarcity_report.pdf") and os.path.getsize("water_scarcity_report.pdf") > 0:
    with open("water_scarcity_report.pdf", "rb") as f:
        ...
        href = f'<a href="data:application/pdf;base64,{b64}" download="water_scarcity_report.pdf">Download Report</a>'
        st.markdown(href, unsafe_allow_html=True)
else:
    st.warning("Report PDF not found. Please compile water_scarcity_report.tex to generate it.")